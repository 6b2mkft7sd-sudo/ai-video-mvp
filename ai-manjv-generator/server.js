...
const STORYBOARD_FILE = path.join(OUTPUT_DIR, "demo-storyboard.txt");
const VIDEO_ASSET_DIR = path.join(OUTPUT_DIR, "video-assets");
const VIDEO_FILE = path.join(OUTPUT_DIR, "storyboard-video.mp4");
const VIDEO_SCRIPT = path.join(__dirname, "scripts", "images-to-video.swift");
...
function writeStoryboardFile(userPrompt, storyboards, modeLabel) {
  ...
  fs.writeFileSync(STORYBOARD_FILE, lines.join("\n"), "utf8");
}
...
async function handleGenerateVideo(req, res) {
  try {
    const { storyboards } = await readRequestJson(req);
    const imageUrls = Array.isArray(storyboards)
      ? storyboards.map(item => item.imageUrl).filter(Boolean)
      : [];

    if (imageUrls.length === 0) {
      sendJson(res, 400, { error: "请先生成至少一张分镜图片" });
      return;
    }

    fs.mkdirSync(VIDEO_ASSET_DIR, { recursive: true });
    fs.rmSync(VIDEO_ASSET_DIR, { recursive: true, force: true });
    fs.mkdirSync(VIDEO_ASSET_DIR, { recursive: true });

    const imagePaths = [];
    for (let index = 0; index < imageUrls.length; index += 1) {
      const imagePath = path.join(VIDEO_ASSET_DIR, `frame-${String(index + 1).padStart(2, "0")}.png`);
      await downloadImageToFile(imageUrls[index], imagePath);
      imagePaths.push(imagePath);
    }

    if (await commandExists("ffmpeg")) {
      const concatFile = path.join(VIDEO_ASSET_DIR, "images.txt");
      const concatLines = [];
      for (const imagePath of imagePaths) {
        concatLines.push(`file '${imagePath.replaceAll("'", "'\\''")}'`);
        concatLines.push("duration 2");
      }
      concatLines.push(`file '${imagePaths[imagePaths.length - 1].replaceAll("'", "'\\''")}'`);
      ...
      await runCommand("ffmpeg", [
        ...
        "-safe", "0",
        "-i", concatFile,
        "-vf", "fps=30,scale=720:1280:force_original_aspect_ratio=increase,crop=720:1280,format=yuv420p",
        "-c:v", "libx264",
        "-movflags", "+faststart",
        VIDEO_FILE
      ]);
    } else if (fs.existsSync("/usr/bin/swift")) {
      await runCommand("/usr/bin/swift", [VIDEO_SCRIPT, VIDEO_FILE, "2", ...imagePaths]);
    } else {
      throw new Error("服务器缺少 ffmpeg，无法生成 mp4 视频");
    }

    sendJson(res, 200, {
      videoUrl: `/storyboard-video.mp4?t=${Date.now()}`,
      downloadUrl: "/storyboard-video.mp4",
      imageCount: imagePaths.length
    });
  } catch (error) {
    console.error("[Video] 生成视频失败", {
      message: error.message,
      stack: error.stack
    });
    sendJson(res, 500, { error: error.message || "视频生成失败" });
  }
}
...
async function handleGenerateStoryboards(req, res) {
  try {
    ...
    if (aiConfig.apiKey) {
      try {
        ...
        if (aiConfig.provider === "dashscope") {
          sendJson(res, 502, {
            error: error.message,
            mode: "dashscope_failed",
            statusMessage: "通义千问调用失败，请查看后端终端完整日志"
          });
          return;
        }
    ...
    writeStoryboardFile(prompt, storyboards, statusMessage);
    ...
    sendJson(res, 200, {
      ...
      video: {
        status: "ready",
        title: "30 秒模拟漫剧视频",
        downloadUrl: "/demo-storyboard.txt"
      }
    });
  } catch (error) {
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "GET" && req.url.startsWith("/storyboard-video.mp4")) {
    fs.readFile(VIDEO_FILE, (error, content) => {
      if (error) {
        res.writeHead(404, { "Content-Type": "text/plain; charset=utf-8" });
        res.end("还没有生成视频，请先点击“生成视频”。");
        return;
      }
      res.writeHead(200, {
        "Content-Type": "video/mp4",
        "Content-Length": content.length,
        "Content-Disposition": "inline; filename=\"storyboard-video.mp4\""
      });
      res.end(content);
    });
    return;
  ...
  if (req.method === "GET" && req.url === "/demo-storyboard.txt") {
    fs.readFile(STORYBOARD_FILE, "utf8", (error, content) => {
      if (error) {
        res.writeHead(404, { "Content-Type": "text/plain; charset=utf-8" });
        res.end("还没有生成分镜文案，请先在页面点击“生成分镜”。");
        return;
      }
