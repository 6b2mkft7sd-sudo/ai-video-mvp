...
- API Key 填写位置：`.env`
- 后端真实模型调用代码：`server.js` 里的 `generateWithDashScope` 和 `handleGenerateStoryboards` 函数
...
Vercel Serverless 环境不能把生成结果持久写到项目目录，也不能依赖 `/var/task/generated` 这类本地目录。因此当前部署版做了以下处理：

- `/api/storyboards` 只返回 JSON，不创建 `generated/`。
- 分镜文案通过接口返回给前端，下载按钮用浏览器内存 Blob 生成 `demo-storyboard.txt`。
- Vercel 环境默认跳过通义万相图片生成，页面显示前端占位图，避免长任务和本地文件写入。
- “生成视频”先返回模拟视频结果，不调用本地 `ffmpeg`、Swift，也不生成 mp4 文件。
...
不要在 Vercel 里上传 `.env` 文件，直接在 Project Settings → Environment Variables 里填写。

如果你后续想在非 Serverless 环境继续尝试通义万相图片，可以在 Render/Railway 里额外设置：

```bash
ENABLE_WANX_IMAGES=true
WANX_ENDPOINT=https://dashscope.aliyuncs.com/api/v1/services/aigc/text2image/image-synthesis
WANX_TASK_ENDPOINT=https://dashscope.aliyuncs.com/api/v1/tasks
WANX_MODEL=wanx-v1
WANX_SIZE=720*1280
...
Render 或 Railway 也可以部署当前 MVP。这个项目保留了 `Dockerfile`，但当前无文件写入版默认不在服务器本地生成 mp4，视频区域先返回模拟结果和分镜文案下载。
...
- 免费实例可能会因为图片生成耗时较长而超时或休眠；如果多人使用，建议升级到付费实例。
- 生产版建议接对象存储 OSS/S3 保存图片和视频，不要依赖 Serverless 本地硬盘。
